...
class Chatbot:
    ...
    def __init__(self, ignoreWords : list = ['?', '!'], verbose : bool = False, forceSave : bool = False, talk : bool = False, SpeechRecognition : bool = False):
        self.roots = []
        self.ruleList = []
        self.corpus = []
        ...
        while True:
            if self.SpeechRecognition:
                with sr.Microphone() as source:
                    ...
                    sentence = ""
                    try:
                        sentence = self.r.recognize_google(audio, language="fr-FR")
                        print("Google pense que vous dites ca : " + sentence)
                    except sr.UnknownValueError:
                        print("Google n'a pas pu comprendre l'audio")
                    except sr.RequestError as e :
                        print("Impossible de faire une requete au serveurs google. Etes vous bien connecté à internet ?")
            else :
                sentence = input('> ')
                if sentence == '.':
                    break
...
if __name__ == '__main__':
    chatbot = Chatbot(verbose=True, talk=True, SpeechRecognition=True)
